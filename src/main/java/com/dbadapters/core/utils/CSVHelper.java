package com.dbadapters.core.utils;
...
public class CSVHelper {
	...
	public static List<Opportunity> csvToOpportunities(InputStream is) {
		try (BufferedReader fileReader = new BufferedReader(new InputStreamReader(is, "UTF-8"));
			...
			for (CSVRecord csvRecord : csvRecords) {
				Opportunity opportunity = new Opportunity(csvRecord.get("OPPORTUITY_NAME"),
						csvRecord.get("ACCOUNT_NAME"), csvRecord.get("EXP_BOOK_DATE"), csvRecord.get("EXP_PRODUCT"),
						csvRecord.get("EXP_SERVICE"), csvRecord.get("EXIST_OPP_NAME"), csvRecord.get("OPP_URL"));
	 ...
	 * csvPrinter = new CSVPrinter(new PrintWriter(out), format);) { for (Employee
	 * tutorial : tutorials) { List<String> data = Arrays.asList(
